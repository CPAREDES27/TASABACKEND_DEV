        ...
        if(fields.length>=1) {
                    ...
                    for (int k = 0; k < fields.length; k++) {
                        ...
                        if (fields[k].trim().equals(key.trim())) {
                           ...
                           try {
                            ...
                            newRecord.put(key, value);
                           Metodos me=new Metodos();
                            if(key.equals("INPRP") || key.equals("ESREG") || key.equals("CDMMA") || key.equals("CDTEV")|| key.equals("STELL")|| key.equals("CDPTA")){
            ...
            for (int i = 0; i < jcoTable.getNumRows(); i++) {
                ...
                for (int j = 0; j < jcoFields.getNumRows(); j++) {
                    ...
                    newRecord.put(key, value);
                    Metodos me=new Metodos();
                    if(key.equals("INPRP") || key.equals("ESREG") || key.equals("STELL") || key.equals("CDPTA")){
                        HashMap<String, Object>dominio=me.BuscarNombreDominio(key, value.toString());
                        for (Map.Entry<String, Object> entry:dominio.entrySet() ){
                            String campo=entry.getKey();
                            Object valor=entry.getValue();
