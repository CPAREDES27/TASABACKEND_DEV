...
public class AceitesUsadosRest {
    ...
    @PostMapping(value = "/Anular", produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<AceitesUsadosExports> Anular(@RequestBody AceitesUsadosImports imports) {

        try {
            return Optional.ofNullable(this.jcoAceitesUsadosService.Anular(imports))
                    .map(l -> new ResponseEntity<>(l, HttpStatus.OK)).orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
        } catch (Exception e) {
            //String error = Utils.obtieneMensajeErrorException(e);
            throw new RuntimeException(e.toString());
